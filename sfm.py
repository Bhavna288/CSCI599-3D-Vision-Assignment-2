...
def set_arguments(parser):
    ...
    # directory stuff
    parser.add_argument('--data_dir', action='store', type=str, default='./assets/Benchmarking_Camera_Calibration_2008', dest='data_dir',
    ...
    parser.add_argument('--out_dir', action='store', type=str, default='./assets/results/', dest='out_dir',
...
class SFM(object):
    def __init__(self, opts):
        ...
        self.feat_dir = os.path.join(
            opts.data_dir, opts.dataset, 'features', opts.features)
        self.matches_dir = os.path.join(
            opts.data_dir, opts.dataset, 'matches', opts.matcher)
        self.out_cloud_dir = os.path.join(
        ...
        if opts.calibration_mat == 'benchmark':
            self.K = np.array(
                [[2759.48, 0, 1520.69], [0, 2764.16, 1006.81], [0, 0, 1]])
    ...
    def baseline_pose_estimation(self, name1, name2):
        ...
        img1pts, img2pts, img1idx, img2idx = self.get_aligned_matches(
            kp1, desc1, kp2, desc2, matches)
        ...
        self.image_data[name1] = [np.eye(3, 3), np.zeros(
            (3, 1)), np.ones((len(kp1),))*-1]
        self.image_data[name2] = [R, t, np.ones((len(kp2),))*-1]

        self.matches_data[(name1, name2)] = [
            matches, img1pts[mask], img2pts[mask], img1idx[mask], img2idx[mask]]
    ...
    def triangulate_two_views(self, name1, name2):
        ...
        _, img1pts, img2pts, img1idx, img2idx = self.matches_data[(
            name1, name2)]

        new_point_cloud = triangulation(img1pts, img2pts, R1, t1, R2, t2)
        self.point_cloud = np.concatenate(
            (self.point_cloud, new_point_cloud), axis=0)
    ...
    def trangulate_new_view(self, name):
        """
        Triangulates new view based on matches with previous views, incorporating outlier removal.
        ...
        for prev_name in self.image_data.keys():
            if prev_name != name:
                ...
                matches = [
                    match for match in matches if prev_name_ref[match.queryIdx] < 0]
                ...
                if len(matches) > 0:
                    ...
                    inlier_matches = [matches[i]
                                      for i in range(len(matches)) if mask[i, 0]]

                    if len(inlier_matches) > 0:
                        img1pts, img2pts = pts1[mask.ravel(
                        ) == 1], pts2[mask.ravel() == 1]
                        ...
                        P2 = self.K.dot(np.hstack((R2, t2)))

                        # Triangulate points using inliers
                        points4D = cv2.triangulatePoints(
                            P1, P2, img1pts.T, img2pts.T)
                        points3D = cv2.convertPointsFromHomogeneous(
                            points4D.T).reshape(-1, 3)

                        # Update the point cloud and references for the new view
                        new_points_start = self.point_cloud.shape[0]
                        self.point_cloud = np.vstack(
                            (self.point_cloud, points3D))
                        self.update_references(
                            name, inlier_matches, new_points_start)
                    else:
                        print(
                            f'Skipping {prev_name} and {name} due to no inlier matches.')

                else:
                    print(
                        f'Skipping {prev_name} and {name} due to no matches.')

    def update_references(self, name, inlier_matches, new_points_start):
        """
        Update references for the new view with newly triangulated points.

        Args:
            name (str): Name of the new view.
            inlier_matches (list): List of inlier matches after RANSAC filtering.
            new_points_start (int): Starting index for new points in the point cloud.

    """
        if name not in self.image_data:
            print(f"Image data for {name} not found")
            return

        ref_indices = self.image_data[name][-1]
        for i, match in enumerate(inlier_matches):
            if match.trainIdx < len(ref_indices):
                ref_indices[match.trainIdx] = new_points_start + i
            else:
                # Handle the case where trainIdx is out of bounds
                print(f"Index {match.trainIdx} out of bounds for image {name}")
        self.image_data[name][-1] = ref_indices
    ...
    def new_view_pose_estimation(self, name):
        ...
        _, R, t, _ = cv2.solvePnPRansac(pts3d[:, np.newaxis], pts2d[:, np.newaxis], self.K, None,
                                        confidence=self.opts.pnp_prob, flags=getattr(
                                            cv2, self.opts.pnp_method),
    ...
    def compute_reprojection_error(self, name):
        ...
        # Convert ref to an array of integers if it's not already
        ref = np.array(ref, dtype=int)

        # Filter out invalid indices (e.g., where ref < 0)
        valid_indices = [i for i, ref_idx in enumerate(ref) if ref_idx >= 0]

        # Now, make sure we are using valid indices to access points in point_cloud
        if not valid_indices:
            print(
                f"No valid 3D references for image {name}. Cannot compute reprojection error.")
            return -1

        valid_indices = np.array(valid_indices)
        points_3d = self.point_cloud[ref[valid_indices]]

        # Project 3D points to 2D using the current camera pose
        projected_points_2d, _ = cv2.projectPoints(
            points_3d, R, t, self.K, distCoeffs=None)

        # Extract the corresponding 2D keypoints
        observed_points_2d = np.float32([kp[i].pt for i in valid_indices])

        # Compute the reprojection error
        errors = np.linalg.norm(
            projected_points_2d.reshape(-1, 2) - observed_points_2d, axis=1)
        err = np.mean(errors)
    ...
    def run(self):
        ...
        print('Baseline Cameras {0}, {1}: Pose Estimation [time={2:.3}s]'.format(
            name1, name2, this_time))

        self.triangulate_two_views(name1, name2)
        t1 = time()
        this_time = t1-t2
        total_time += this_time
        print('Baseline Cameras {0}, {1}: Baseline Triangulation [time={2:.3}s]'.format(
            name1, name2, this_time))

        views_done = 2

        # 3d point cloud generation and reprojection error evaluation
        self.generate_ply(os.path.join(self.out_cloud_dir,
                          'cloud_{}_view.ply'.format(views_done)))
        ...
        for new_name in self.image_names[2:]:
            ...
            this_time = t2-t1
            total_time += this_time
            print('Camera {0}: Pose Estimation [time={1:.3}s]'.format(
                new_name, this_time))
            ...
            this_time = t1-t2
            total_time += this_time
            print('Camera {0}: Triangulation [time={1:.3}s]'.format(
                new_name, this_time))
        ...
        print('Reconstruction Completed: Mean Reprojection Error = {2} [t={0:.6}s], Results stored in {1}'.format(
            total_time, self.opts.out_dir, mean_error))
