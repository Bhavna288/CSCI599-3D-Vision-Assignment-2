...
def serialize_keypoints(kp):
    ...
    out = []
    for kp_ in kp:
        temp = (kp_.pt, kp_.size, kp_.angle,
                kp_.response, kp_.octave, kp_.class_id)
        out.append(temp)
...
def deserialize_keypoints(kp):
    ...
    out = []
    for point in kp:
        temp = cv2.KeyPoint(x=point[0][0], y=point[0][1], size=point[1],
                            angle=point[2], response=point[3], octave=point[4], class_id=point[5])
        out.append(temp)
...
def serialize_matches(matches):
    ...
    for match in matches:
        matchTemp = (match.queryIdx, match.trainIdx,
                     match.imgIdx, match.distance)
...
def draw_correspondences(img, ptsTrue, ptsReproj, ax, drawOnly=50):
    ...
    # Randomly select points to draw if there are more points than drawOnly
    if len(ptsTrue) > drawOnly:
        indices = np.random.choice(len(ptsTrue), size=drawOnly, replace=False)
    else:
        indices = np.arange(len(ptsTrue))
    ...
    for i in indices:
        ...
        # Green dot for ground truth
        ax.plot(true_point[0], true_point[1], 'go')

        # Draw the reprojected point
        ax.plot(reproj_point[0], reproj_point[1],
                'ro')  # Red dot for reprojected

        # Draw a line between the true point and the reprojected point
        ax.plot([true_point[0], reproj_point[0]], [
                true_point[1], reproj_point[1]], 'y-')  # Yellow line
